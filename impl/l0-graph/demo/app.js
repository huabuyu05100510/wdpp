...
function renderOrders() {
  ...
  for (const order of appState.orders) {
    ...
    // 每个字段单独 span,触发独立 __readSlot
    const idSpan = document.createElement('span');
    idSpan.className = 'order-id';
    idSpan.textContent = `#${__readSlot(order, 'id')}`;

    const productSpan = document.createElement('span');
    productSpan.className = 'order-product';
    productSpan.textContent = __readSlot(order, 'product');

    const priceSpan = document.createElement('span');
    priceSpan.className = 'order-price';
    priceSpan.textContent = `$${__readSlot(order, 'price')}`;

    const statusSpan = document.createElement('span');
    statusSpan.className = 'order-status';
    statusSpan.textContent = __readSlot(order, 'status');

    li.append(idSpan, ' - ', productSpan, ' - ', priceSpan, ' - ', statusSpan);
    list.appendChild(li);
  }
}
...
function renderProducts() {
  ...
  for (const p of appState.products) {
    ...
    nameSpan.textContent = __readSlot(p, 'name');

    const priceSpan = document.createElement('span');
    priceSpan.className = 'product-price';
    priceSpan.textContent = `$${__readSlot(p, 'price')}`;

    const stockSpan = document.createElement('span');
    stockSpan.className = 'product-stock';
    stockSpan.textContent = `(${__readSlot(p, 'stock')} in stock)`;

    li.append(nameSpan, ' - ', priceSpan, ' ', stockSpan);
    list.appendChild(li);
  }
}

...
const PROJECT_ROOT = resolve(__dirname, '..');  // impl/l0-graph/
const PORT = 8080;
...
const server = createServer((req, res) => {
  ...
  // 静态文件(支持 /src/、/demo/、/test/ 路径)
  let filePath;
  if (pathname === '/') {
    filePath = join(__dirname, 'index.html');
  } else if (pathname.startsWith('/src/') || pathname.startsWith('/babel/')) {
    filePath = join(PROJECT_ROOT, pathname);
  } else if (pathname.startsWith('/demo/') || pathname.startsWith('/test/')) {
    filePath = join(PROJECT_ROOT, pathname);
  } else {

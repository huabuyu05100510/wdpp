...
> **重要说明**:本文讲的是 **WDPP v1(按值索引)**。从 2025 年起,WDPP 已升级为 **v2(双引擎:按值 + 按图)**。按图架构的深度内容请看《WDPP 按图架构(工程师视角)》一文。
...
| 文档 | 视角 | 目标读者 | 内容 |
|---|---|---|---|
| **本文**:WDPP 技术博客(深入浅出) | 科普 | 刚毕业 / 入行 1-2 年 | v1 按值索引 + Babel 插桩 |
| [按图架构(工程师视角)]() | 工程师 | 5 年+ 经验 | v2 双引擎 + 图架构 + 性能数据 |
| [开源生态对比(深入浅出)]() | 选型 | 架构师 / 决策者 | vs Chrome DevTools / Sentry / OpenLineage 等 |
| [WDPP 2.0 测试结果]() | 评审 | 评审者 | 255 测试 + 覆盖率 + 性能数据 |

**阅读路径**:
- 想知道"WDPP 是什么" → 读本文
- 想用 WDPP 做事 / 排查问题 → 读按图架构
- 想选型 / 评估能否替代现有工具 → 读开源生态对比
- 评审 / PR / 性能基线 → 读测试结果
